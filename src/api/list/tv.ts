...
async function geTv(url: string): Promise<episode[] | null> {
  ...
  doc1.querySelectorAll("a").forEach((x: Record<string, any>) => {
    ...
    if (x.previousElementSibling?.innerHTML?.includes("Episódio")) {
      ...
      x.parentElement.previousElementSibling?.innerHTML?.includes("Episódio")
    ) {
      nameInner = x.parentElement?.previousElementSibling?.innerHTML?.replace(
        /[^0-9]/g,
        ""
      );
    } else if (
      x.parentElement.previousElementSibling?.previousElementSibling?.innerHTML?.includes(
        "Episódio"
      )
    ) {
      nameInner =
        x.parentElement?.previousElementSibling?.previousElementSibling?.innerHTML?.replace(
          /[^0-9]/g,
          ""
        );
    }
    if (
      ...
      if (nameInner) {
        if (
          (nameInner.includes("1") && nameInner.length === 1) ||
          (nameInner.includes("01") && nameInner.length === 2) ||
          (nameInner.includes("001") && nameInner.length === 3)
        ) {
          count++;
        }
        response.push({
      ...
      const episodeNumber = x.previousElementSibling.innerHTML
        .replace("Episódio ", "")
        .replace(" - ", "");
      if (episodeNumber === "01") {
        count++;
...
export default async function infoTvList(
  item: DataTv
